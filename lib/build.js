...
const fs = require('fs');
const transform = require('mt-transfomer');
const noop = function () {
};
...
class Build {
    ...
    run(isUpload) {
        ...
        let options = {
            listeners: {
                file: function (root, fileStats, next) {
                    ...
                    let ignore = config.compile.ignore || '';
                    let ignoreExp = ignore ? new RegExp(ignore) : /(\.svn|\.DS_Store$|\.git|\.idea|\.tmp|node_modules)/g;
        ...
        if (config.common) {
            log(config.common);
            transform.transformMods({repository: config.localFrontendMods, output: config.common.output});
        }

        if (config.build.indexOf('css') > 0) {
